# sensors/vision/camera_service.py

import cv2, os, time, threading, json, shutil, uuid, socket
...
class CameraService:
    ...
    def _start_rpc(self):
        ...
        os.chmod(SOCKET_PATH, 0o660)            # vision group
        srv.listen()
        while True:
            conn, _ = srv.accept()
    ...
    def _handle_client(self, conn):
        ...
        try:
            ...
            if method == 'read':
                frame_id, _ = self.read()          # ignore matrix
                # return only the filepath
                result = {
                    'frame_id': frame_id,
                    'path':     os.readlink(LATEST_SYMLINK)
                }
                resp = {'jsonrpc':'2.0', 'result':result, 'id':rid}

            elif method == 'mark_persistent':
                self.mark_persistent(params['frame_id'], params.get('meta', {}))
                resp = {'jsonrpc':'2.0', 'result':'ok', 'id':rid}
